                  ...
                  AnimatedSwitcher(
                    ...
                    child: _sent
                        ...
                        : _FormView(key: const ValueKey('f'), emailCtrl: _email, error: _error,
                            isSubmitting: _isSubmitting, onSubmit: _sendReset),
                  ),
                ]),
              ),
            ),
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      AuthField(label: 'Email Address', hint: 'you@example.com', controller: emailCtrl,
        ...
        onSubmitted: (_) => onSubmit(), prefixIcon: Icons.mail_outline_rounded),
      AuthErrorBanner(error: error),
      const SizedBox(height: AppSpacing.s24),
      AuthPrimaryButton(label: 'Send Reset Link', isLoading: isSubmitting, onPressed: onSubmit),
    ]);
  }
}
...
class _SuccessView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(width: 52, height: 52,
        decoration: BoxDecoration(color: const Color(0xFF30A46C).withOpacity(0.15),
      ...
      const SizedBox(height: AppSpacing.s12),
      Text.rich(TextSpan(
        style: AppTextStyles.body.copyWith(color: AppColors.onDarkMuted, height: 1.6),
        children: [
          const TextSpan(text: "We've sent a password reset link to "),
          TextSpan(text: email, style: AppTextStyles.body.copyWith(color: Colors.white, fontWeight: FontWeight.w500)),
          const TextSpan(text: '.'),
        ])),
      const SizedBox(height: AppSpacing.s12),
      Text("Didn't receive it? Check your spam folder.",
        style: AppTextStyles.caption.copyWith(color: AppColors.onDarkMuted.withOpacity(0.7))),
      const SizedBox(height: AppSpacing.s40),
      AuthPrimaryButton(label: 'Back to Sign In', isLoading: false, onPressed: onBack),
    ]);
  }
}
