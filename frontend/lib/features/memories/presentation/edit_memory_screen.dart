...
class _EditMemoryScreenState extends ConsumerState<EditMemoryScreen> {
  ...
  void initState() {
    ...
    _locController = TextEditingController(text: widget.memory.locationName ?? '');
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      await mRepo.updateMemory(
        ...
        locationName: _locController.text.trim().isEmpty ? null : _locController.text.trim(),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Edit Memory', style: TextStyle(color: c.text, fontWeight: FontWeight.w600, fontSize: 16)),
        leading: IconButton(
        ...
        actions: [
          _isSaving
              ? const Padding(padding: EdgeInsets.all(16.0), child: SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)))
              : TextButton(
                  onPressed: _saveChanges,
                  child: Text('Save', style: TextStyle(color: c.primary, fontWeight: FontWeight.w600)),
                ),
        ],
      ),
      body: Form(
                        ...
                        Expanded(
                          child: InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(color: c.surfaceElevated, borderRadius: BorderRadius.circular(AppRadii.md)),
                              child: Row(
                                children: [
                                  Icon(Icons.calendar_today, size: 18, color: c.textMuted),
                                  const SizedBox(width: 12),
                                  Text(DateFormat('MMM d, yyyy').format(_selectedDate), style: TextStyle(color: c.text)),
                                ],
                              ),
                            ),
                          ),
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              prefixIcon: Icon(Icons.location_on_outlined, size: 18, color: c.textMuted),
                              filled: true,
                              fillColor: c.surfaceElevated,
                              border: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadii.md), borderSide: BorderSide.none),
                            ),
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text('Media', style: TextStyle(color: c.text, fontSize: 16, fontWeight: FontWeight.w600)),
                    TextButton.icon(
                      ...
                      onPressed: () {
                        ...
                        MultiMediaPickerScreen.open(context, memoryId: widget.memory.id);
                      },
                    ),
                  ],
                ),
            ...
            if (_mediaList.isEmpty)
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Center(
                    child: Text('No media attached.', style: TextStyle(color: c.textMuted)),
                  ),
                ),
              )
            else
              SliverReorderableList(
                itemBuilder: (context, index) {
                  ...
                  return Container(
                    ...
                    margin: const EdgeInsets.symmetric(horizontal: AppSpacing.s24, vertical: AppSpacing.s4),
                    decoration: BoxDecoration(
                      color: c.surfaceElevated,
                      borderRadius: BorderRadius.circular(AppRadii.sm),
                      border: isCover ? Border.all(color: c.primary, width: 2) : null,
                    ),
                    child: ListTile(
                      ...
                      leading: ClipRRect(
                        ...
                        child: Image.network(
                          ...
                          height: 48,
                          fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => Container(width: 48, height: 48, color: c.surface),
                        ),
                      ),
                      title: Text(media.mediaType == 'video' ? 'Video' : 'Photo', style: TextStyle(color: c.text, fontSize: 14)),
                      subtitle: isCover ? Text('Cover', style: TextStyle(color: c.primary, fontSize: 12)) : null,
                      trailing: Row(
                        ...
                        children: [
                          PopupMenuButton<String>(
                            ...
                            color: c.surface,
                            onSelected: (val) {
                              if (val == 'cover') setState(() => _coverMediaId = media.id);
