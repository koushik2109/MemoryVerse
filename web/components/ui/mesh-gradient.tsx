...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const w = Math.max(1, Math.round(rawW * ps))
      const h = Math.max(1, Math.round(rawH * ps))
    ...
    const updatePointerTarget = () => {
      ...
      targetX = nx; targetY = ny; targetPresence = 1; requestRender()
    }
    const onPointerMove = (e: PointerEvent) => { pointerKnown = true; pointerClientX = e.clientX; pointerClientY = e.clientY; bounds = canvas.getBoundingClientRect(); updatePointerTarget() }
